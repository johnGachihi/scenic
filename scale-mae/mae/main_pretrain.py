...
wandb_apikey = os.getenv('WANDB_API_KEY_SAT_LOCA', '')
...
def main(args):
    ...
    if not args.eval_only:
        ...
        if len(args.target_size) > 0:
            ...
            assert all(
                [i % args.patch_size == 0 for i in args.target_size]
            ), f"Decoder resolution must be a multiple of patch size ({args.patch_size})"
        ...
        elif config["data"]["type"] in ["mmearth"]:
            transforms_init = tv_transforms.Compose([
                tv_transforms.RandomCrop(args.input_size * 2, padding=0, pad_if_needed=True, fill=0),
                tv_transforms.RandomHorizontalFlip(),
            ])
            other_transforms = None
        else:
    ...
    if misc.is_main_process() and not args.eval_only:
        ...
        wandb.login(key=wandb_apikey, relogin=True)
