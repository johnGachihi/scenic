...
class MMEarthDataset(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    for modality in self.modalities.keys():
      ...
      data = (np.where(data == MODALITIES.NO_DATA_VAL[modality], np.nan, data) if modality != "dynamic_world" else data )
...
def create_MMEearth_args(modalities: dict) -> Namespace:
  args = Namespace()
...
class MMEarthDatasetFromTFDS(Dataset):
  ...
  def __init__(self, tf_dataset: Dataset):
  ...
  def __getitem__(self, idx):
    ...
    if sentinel2_type == "l2a":
      sample['sentinel2'] = (sample['sentinel2'] - self.SENTINEL2_L2A_MEAN[:, None, None]) / self.SENTINEL2_L2A_STD[:, None, None]
    else:
      sample['sentinel2'] = (sample['sentinel2'] - self.SENTINEL2_L1C_MEAN[:, None, None]) / self.SENTINEL2_L1C_STD[:, None, None]

    # set consistent no-data value across modalities
    sample['sentinel2'] = np.where(
      sample['sentinel2'] == MODALITIES.NO_DATA_VAL['sentinel2'],
      np.nan,
      sample['sentinel2'])
    sample['sentinel1'] = np.where(
      sample['sentinel1'] == MODALITIES.NO_DATA_VAL['sentinel1'],
      np.nan,
      sample['sentinel1'])

    return {
      'sentinel2': sample['sentinel2'].astype(np.dtype('float32')),
      'sentinel1': sample['sentinel1'].astype(np.dtype('float32'))
    }
